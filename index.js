...
function generatePassword() {
    const passwordLengthSelector = getPasswordLengthSelector();
    let passwordLength = Number(passwordLengthSelector.value);
    ...
    const specialCharactersQuantitySelector = getSpecialCharactersQuantitySelector();
    ...
    if (specialCharactersCheckbox.checked) {
        passwordRules.push({
            characters: '!@#$&*?|%+-_./:;=()[]{}',
            min: Number(specialCharactersQuantitySelector.value)
        });
    }
    ...
    if (allMin > passwordLength) {
        alert(`The rules you chose require more characters (${allMin}) than the password length (${passwordLength}). Please adjust your configurations.`);
        return;
    }
    ...
    // Ensure the generated password does not exceed the specified length
